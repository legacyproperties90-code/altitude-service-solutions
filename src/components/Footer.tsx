    ...
    <footer className="bg-[#060E1E] border-t border-blue-900/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              Professional exterior cleaning services in Atlanta, GA. Fully insured with 100% satisfaction guarantee.
            </p>
            <div className="flex gap-3">
              <a href="https://facebook.com" target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg bg-blue-900/30 flex items-center justify-center text-blue-400 hover:bg-blue-600 hover:text-white transition-all">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
              </a>
              <a href="https://instagram.com" target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-lg bg-blue-900/30 flex items-center justify-center text-blue-400 hover:bg-pink-600 hover:text-white transition-all">
          ...
          {/* Residential */}
          <div>
            <h4 className="text-white font-semibold mb-5">Residential</h4>
            <ul className="space-y-2.5">
              {["Soft Washing", "Pressure Washing", "House Washing", "Roof Cleaning", "Fence & Deck", "Gutter Cleaning", "Window Cleaning"].map((s) => (
                <li key={s}>
                  <Link href={`/residential#${s.toLowerCase().replace(/\s+/g, "-")}`}
                    className="text-gray-400 hover:text-blue-400 text-sm transition-colors">{s}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Commercial */}
          <div>
            <h4 className="text-white font-semibold mb-5">Commercial</h4>
            <ul className="space-y-2.5">
              {["Pressure Washing", "Soft Washing", "Parking Deck Cleaning", "High-Rise Window Cleaning", "Waterproofing"].map((s) => (
                <li key={s}>
                  <Link href={`/commercial#${s.toLowerCase().replace(/\s+/g, "-")}`}
                    className="text-gray-400 hover:text-blue-400 text-sm transition-colors">{s}</Link>
                </li>
              ))}
            </ul>
            ...
            <ul className="space-y-4">
              ...
              <li>
                <a href="mailto:info@altitudeservicesolutions.com" className="flex items-start gap-3 text-gray-400 hover:text-blue-400 text-sm transition-colors">
                  ...
                  info@altitudeservicesolutions.com
                </a>
              </li>
              <li>
      ...
      <div className="border-t border-blue-900/20 py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row justify-between items-center gap-3">
          <p className="text-gray-500 text-sm">© {new Date().getFullYear()} Altitude Service Solutions, LLC. All rights reserved.</p>
          <p className="text-gray-600 text-xs">Serving Atlanta, Lawrenceville & surrounding areas</p>
        </div>
      </div>
    </footer>
  );
