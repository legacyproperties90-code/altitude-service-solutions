  ...
  return (
    <section className="relative w-full min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 sm:px-10 pt-24 pb-20 text-center">
        ...
        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-black leading-tight mb-5">
          {headline}{" "}{highlight}
        </h1>

        <p className="text-base sm:text-lg text-gray-900 max-w-xl mx-auto mb-8 leading-relaxed font-semibold">
          {subtitle}
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center mb-10">
          ...
          <a href="tel:+16787395229"
            className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl border-2 border-white/60 bg-black/30 backdrop-blur-sm text-white font-bold text-base hover:bg-black/50 transition-all"
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
        {slides.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)} aria-label={`Slide ${i + 1}`}
            className={`rounded-full transition-all duration-300 ${i === current ? "w-8 h-2.5 bg-blue-400" : "w-2.5 h-2.5 bg-white/50 hover:bg-white/80"}`}
          />
        ))}
      </div>
    </section>
